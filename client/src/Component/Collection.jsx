...
const AllProduct = () => {
  ...
  // Map of sort options to query parameters
  const sortParams = {
    featured: "-createdAt",
    "title-asc": "title",
    "title-desc": "-title",
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const sortParam = sortParams[sortOption];
        const response = await fetch(
          `http://localhost:4000/api/product?sort=${sortParam}&page=${currentPage}&limit=${limit}`
        );
